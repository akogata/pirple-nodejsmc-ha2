...
handlers._menu.get = (data, callback) => {
  const email = _validEmail(data.queryStringObject.email)
  if (!email) {
    callback(400, { 'Error': 'Missing required field \'email\'' }) // eslint-disable-line standard/no-callback-literal
    return
  }
  const tokenId = _validHeaderToken(data.headers.token)
  if (!tokenId) {
  ...
  _data.read('tokens', tokenId, (err, tokenData) => {
    ...
    if (email !== tokenData.email) {
      callback(400, { 'Error': 'Token does not match email' }) // eslint-disable-line standard/no-callback-literal
      return
    }
...
handlers._orders.get = (inputData, callback) => {
  ...
  const tokenId = _validHeaderToken(inputData.headers.token)
  if (!tokenId) {
    callback(400, { 'Error': 'Missing token in header or invalid token' }) // eslint-disable-line standard/no-callback-literal
    return
  }
  _data.read('tokens', tokenId, (err, tokenData) => {
    ...
    _data.read('users', email, async (err, userData) => {
      ...
      _orderAmount(userData.slices, amountToCharge => {
        debug('handler._order.get: amountToCharge', amountToCharge)
...
handlers._orders.post = (inputData, callback) => {
  ...
  const tokenId = _validHeaderToken(inputData.headers.token)
  if (!tokenId) {
    callback(400, { 'Error': 'Missing required token in header or invalid token' }) // eslint-disable-line standard/no-callback-literal
    return
  }
  _data.read('tokens', tokenId, (err, tokenData) => {
    if (err) {
    ...
    _data.read('users', email, (err, userData) => {
      ...
      _orderAmount(userData.slices, amountToCharge => {
        ...
        _data.update('users', email, userData, updateErr => {
          ...
          const apiUserId = 'sk_test_chBRANAHfXzvM2xtOAdCdNi8'
          const params = {
            amount: (amountToCharge * 100) | 0,
            currency: 'usd',
            description: `Charge for ${userData.name} (${userData.email})`,
            source: 'tok_mastercard'
          }
          helpers.charge(apiUserId, params, (chargeError, response, body) => {
            if (chargeError) {
            ...
            if (response.statusCode !== 200 || !body.paid) {
              debug('helpers.charge: statusCode:', response.statusCode)
            ...
            const emailParams = {
              ...
              text: 'Thanks! Amount charged: ' + amountToCharge + '\nCya!'
            }
            debug('orders.post:', emailParams)
            helpers.sendEmailNotification(emailParams, (emailNotificationError, emailNotificationResponse, emailNotificationBody) =>  {
              debug('helpers.sendEmailNotification.body:', emailNotificationBody)
              if (emailNotificationError) {
                debug('order.post.emailNotificationError', emailNotificationError)
